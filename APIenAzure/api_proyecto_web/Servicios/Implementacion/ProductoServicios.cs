...
namespace api_proyecto_web.Servicios.Implementacion
{
    ...
    public class ProductoServicios : IcrudProductos
    {
        ...
        {
            string query_id = "select id_producto.NEXTVAL AS id FROM DUAL";
            DataTable dt_id = ComprasServicios.db.Execute(query_id);
            Productos productito = new Productos();
            string query1 = string.Format("insert into producto values (" + dt_id.Rows[0]["id"] + ",'{0}','{1}',{2},{3},'T') ", nombre, caracteristicas, id_tipo_Producto, precio);
        ...
        {
            string queryIdImagen = "SELECT sq_imagenes.NEXTVAL AS id from DUAL";
            DataTable dtId = ComprasServicios.db.Execute(queryIdImagen);
            int imagenId = Convert.ToInt32(dtId.Rows[0]["id"]);

            string queryIngresoFoto = string.Format(" insert into imagen values ({0}, '{1}', 2,null,{2}, '{3}', 'T')", imagenId, url, productoId, orden.ToString());
        ...
        public void EliminarProducto(int id)
        {
            ...
            var producto = InformacionProducto(id);
            if (producto == null)
            {
        ...
        private void EliminarImagenesProducto(int productoId)
        {
            string query = $"DELETE FROM imagen WHERE id_producto = {productoId} ";
            ComprasServicios.db.Execute(query);
            string commit1 = string.Format("COMMIT");
            DataTable cm1 = ComprasServicios.db.Execute(commit1);
        }
        ...
        public List<Productos> ObtenerTodosLosProductos()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                producto.estado = row["estado"].ToString() == "T" ? true : false;
                string query_imagenes = "select id_imagen as id_img,id_tipo_imagen as tipo_img,i.nombre as nombre,url_imagen as url from producto p join imagen i on (p.id_producto = i.id_producto) where p.id_producto = " + producto.Id + " and i.estado = 'T'";
                DataTable dt_imagenes = ComprasServicios.db.Execute(query_imagenes);
                if (dt_imagenes.Rows.Count > 0)
                {
                    if (dt_imagenes.Rows.Count == 1)
